...
% function [] = function_mainCDFvsRateSINR_3(rndseed0, rndseed, num, nIter)
tic
...
numCDF = num; % number of fast fading realizations
...
numV2V = 3*numV2I;
%%
sumRateProp = zeros(numCDF,1);
sinrProp = zeros(numCDF,1);
...
while (1)
    ...
    while iChann <= numCDF
        ...
        [sumV2I, x_mfk, Pd_mfk, Pc_mfk] = solveMatching3D(clusterMatProp, ...
            alpha_mB_, alpha_kB_, alpha_mk_, alpha_kk_, sig2, Pd_max, Pc_max, gammaProp, h_mBf_, h_kBf_);
        
        % CDF of V2I capacity
        sumRateProp(iChann) = sumRateProp(iChann) + sumV2I;
        ...
        % sum along f
        x_mk_f = sum(x_mfk,2);
        Pd_mk_f = sum(Pd_mfk,2);
        Pc_mk_f = sum(Pc_mfk,2);
        
        k = 1;
        m = find(x_mk_f(:,k)==1);
        
        Pd = Pd_mk_f(m,x_mk_f(m,:)==1);
        tp = Pc_mk_f(m,x_mk_f(m,:)==1);
        
        kPro = find(x_mk_f(m,:)==1);
        g_mk = alpha_mk_(m,kPro(k)).*abs(randn(1)+1j*randn(1)).^2/2;
        g_kk = alpha_kk_(kPro,kPro).*abs(randn(length(kPro))+1j*randn(length(kPro))).^2/2;
        ...
        sinrProp(iChann) = signal/(sig2+interf);
...
for ind_sinr = 1:length(sinr)
    marker_0(ind_sinr) = sum(sinrProp <= 10^(sinr(ind_sinr)/10))/numCDF;
end
...
% save(sprintf('mainCDFvsRate_3_%d',rndseed))
toc
